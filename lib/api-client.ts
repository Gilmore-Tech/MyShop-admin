...
const SAFE_ERROR_COPY: Readonly<Record<string, string>> = {
  ...
  DRIVER_PRIORITY_ALGORITHM_DOWNGRADE_BLOCKED: 'Priority Driver V2 cannot be changed back to the online-only algorithm. Disable enforcement instead.',
  DRIVER_PRIORITY_V2_SHADOW_REQUIRED: 'Publish the first V2 policy in shadow-only mode with enforcement off and rollout at 0%.',
  DRIVER_PRIORITY_POLICY_EVIDENCE_INVALID: 'Accepted-offer evidence cannot be higher than eligible-offer evidence.',
  DRIVER_PRIORITY_V2_EVALUATION_INCOMPLETE: 'Wait for every driver to receive a completed V2 evaluation before enabling rollout.',
  DRIVER_PRIORITY_SHADOW_REQUIRED: 'Shadow measurement must remain enabled while priority enforcement is active.',
