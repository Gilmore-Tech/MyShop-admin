...
import { paymentMethodLabel, transactionTypeDescription, transactionTypeLabel } from '@/lib/payment-labels'
...
const OUTFLOW_TYPES = new Set(['payout', 'refund'])
            ...
            {loading ? (
              ...
              transactions.map(tx => (
                ...
                >
                  ...
                  <TableCell>
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${txTypeColors[tx.type] ?? 'bg-gray-100 text-gray-600'}`}>
                      {transactionTypeLabel(tx.type)}
                    </span>
                  </TableCell>
                  <TableCell className={`text-sm font-semibold tabular-nums ${OUTFLOW_TYPES.has(tx.type) ? 'text-red-600' : 'text-gray-900'}`}>
          ...
          <>
            ...
            <div className="px-6 py-5 space-y-3 text-sm">
              ...
              <Row
                label="Amount"
                value={
                  <span className={`font-semibold tabular-nums ${OUTFLOW_TYPES.has(tx.type) ? 'text-red-600' : 'text-gray-900'}`}>
                    {formatTransactionAmount(tx.amountPesewas, tx.type)}
                  </span>
                }
              />
              <Row label="Meaning" value={transactionTypeDescription(tx.type)} />
