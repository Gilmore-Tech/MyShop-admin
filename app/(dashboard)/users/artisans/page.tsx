...
function actionCopy(type: ActionType, name: string) {
  if (type === 'ban') {
    return {
      title: `Ban ${name}'s artisan account?`,
      description: 'This bans only the artisan account. Client and driver sibling accounts remain untouched.',
      confirmLabel: `Ban ${name}`,
    }
  }
  if (type === 'reinstate') {
    return {
      title: `Reinstate ${name}'s artisan account?`,
      description: 'Restores only this artisan account. Client and driver sibling accounts remain untouched.',
      confirmLabel: `Reinstate ${name}`,
    }
  }
  return {
    title: `Suspend ${name}'s artisan account?`,
    description: 'This suspends only the artisan account. Client and driver sibling accounts remain untouched.',
...
export default function ArtisansPage() {
  ...
  const fetchUsers = useCallback(() => {
    ...
    listUsers({
      ...
      .catch(err => {
        setUsers([])
        setError(err instanceof ApiError ? err.message : 'Failed to load artisans.')
      })
  ...
  async function handleAction(reason: string) {
    ...
    setActionError('')
    try {
      if (actionType === 'suspend') await suspendUser('artisan', actionUser.roleAccountId, reason)
      else if (actionType === 'ban') await banUser('artisan', actionUser.roleAccountId, reason)
      else if (actionType === 'reinstate') await reinstateUser('artisan', actionUser.roleAccountId, reason || undefined)
      setActionUser(null)
      setActionType(null)
      fetchUsers()
    } catch (err) {
  ...
  return (
    <PageGuard permission="view_users">
      <div>
        <PageHeader
          title="Artisans"
          subtitle="Artisan accounts and standing"
          tabs={<UserTabs active="artisans" />}
        />

        <FilterBar onRefresh={fetchUsers} refreshing={loading} meta={`${total.toLocaleString()} artisans`}>
        ...
        <DataTable<PlatformUser>
          columns={[
            ...
            },
            {
              key: 'verification',
              header: 'Verification',
              render: row => <StatusBadge status={row.role === 'artisan' ? row.profile.verificationStatus : 'pending'} />,
            },
            {
              key: 'online',
              header: 'Online status',
              render: row => row.role === 'artisan' && row.profile.onlineStatus === 'online'
          ...
          rowMenu={row => (
            <>
              ...
              <DropdownMenuItem onSelect={() => router.push(`/artisan-jobs?search=${encodeURIComponent(row.fullName)}`)}>
                Job history
              </DropdownMenuItem>
          ...
          empty={
            <EmptyState
              icon={Wrench}
              title={search || statusFilter !== 'all' ? 'No matches' : 'No artisans yet'}
              description={search || statusFilter !== 'all' ? 'Try a different search or filter.' : 'Registered artisans will appear here.'}
            />
          }
